...
test("7. Stop/Block/Activate proxies", () => {
	...
	proxy.level1_2.level2_1.level3_1.on({
		event: "change",
		deep: true,
		listener: function (changes) {
			numberOfEmits++;
...
test("11. Listen for delete event of sub-properties when parent is deleted", (done) => {
	...
	proxy.on({
		...
		listener: function (change) {
			setTimeout(() => {
				//make sure we run after the other delete event
				counter++;
				if (counter === 3) {
					expect(change.path).toBe("");
