...
test("2. evalPath - get target property of object and path", () => {
	...
	proxy.on({
		...
		listener: function (changes) {
			let { object, property, value } = Proxserve.evalPath(
				this,
				changes[0].path
			);
			expect(
				object === proxy.level1_2.level2_1.level3_1.arr2[2][2][1].deep
			).toBe(true);
	...
	proxy.level1_2.level2_1.on({
		...
		listener: function (changes) {
			...
			expect(object === proxy.level1_2.level2_1.level3_1.arr2[2]).toBe(
				true
			);
...
test("4. Debug Trace", () => {
	...
	expect(
		/test_name\.level1_1\.prim1.+has been created/gi.test(expected[1])
	).toBe(true);
	...
	expect(
		/test_name\.level1_1\.arr1.+has been spliced/gi.test(expected[5])
	).toBe(true);
	...
	expect(
		/test_name\.level1_1\.prim1.+has been deleted/gi.test(expected[1])
	).toBe(true);
	...
	expect(/Stack Trace.*at Object.deleteProperty/gi.test(expected[6])).toBe(
		true
	);
	expect(expected[7].indexOf("border-top")).toBeGreaterThan(3);
	expect(expected[8].indexOf("border-bottom")).toBeGreaterThan(3);
	expect(
		/test_name\.level1_1\.arr1.+has been unshifted/gi.test(expected[9])
	).toBe(true);
