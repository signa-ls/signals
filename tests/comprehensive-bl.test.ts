	...
	proxy.level1_2.level2_1.on({
		...
		listener: function (change) {
			...
			if (counter <= 4) expect(isProxy(this)).toBe(true);
				...
				throw new Error(
					`shouldn't have gotten here on step #${counter}`
				);
		},
		deep: true,
	});
...
test("2. Destroy proxy and sub-proxies", (done) => {
	...
	let level3_proxyNode =
		proxy.level1_2.level2_1.level3_1.getProxserveNodes().proxyNode;
	let arr2_proxyNode =
		proxy.level1_2.level2_1.level3_1.arr2.getProxserveNodes().proxyNode;
	...
	function part3() {
		...
		const sub3_proxyNode =
			proxy.sub1.sub2.sub3.getProxserveNodes().proxyNode;
		const a_proxyNode =
			proxy.sub1.sub2.sub3[0].getProxserveNodes().proxyNode;
		const sub4_proxyNode =
			proxy.sub1.sub2.sub3.sub4.getProxserveNodes().proxyNode;
...
test("3. Transfer sub-proxies between child nodes while assigning new proxies with updated paths", (done) => {
	...
	setTimeout(() => {
		...
		expect(
			arr2_PN[ND].target === proxy.sub_obj.sub_arr[1].getOriginalTarget()
		).toBe(true); //same target
		...
		expect(
			arr3_PN[ND].target === proxy.sub_obj.sub_arr[2].getOriginalTarget()
		).toBe(true); //same target
	...
	function part3() {
		...
		expect(
			array_PN[ND].proxy[0] === arr0_PN[ND].proxy &&
				array_PN[ND].target[0] === arr0_PN[ND].target
		).toBe(true);
		expect(
			array_PN[ND].proxy[1] === arr1_PN[ND].proxy &&
				array_PN[ND].target[1] === arr1_PN[ND].target
		).toBe(true);
		expect(
			array_PN[ND].proxy[2] === arr2_PN[ND].proxy &&
				array_PN[ND].target[2] === arr2_PN[ND].target
		).toBe(true);
		expect(
			array_PN[ND].proxy[3] === arr3_PN[ND].proxy &&
				array_PN[ND].target[3] === arr3_PN[ND].target
		).toBe(true);
		...
		setTimeout(() => {
			...
			expect(
				Array.isArray(array_PN[ND].proxy[3]) &&
					Array.isArray(array_PN[ND].target[3])
			).toBe(true);
			done();
		}, 30);
	}
...
test("5. Find recursively and handle proxies inside objects inserted to main proxy", () => {
	...
	expect(
		proxy.newObj.subObj_1.getOriginalTarget() ===
			proxy.arr[0].getOriginalTarget()
	).toBe(true);
	...
	expect(
		proxy.newObj.subObj_2.getOriginalTarget() ===
			proxy.arr[1].getOriginalTarget()
	).toBe(true);
	...
	expect(
		proxy.newObj.subArr[0].getOriginalTarget() ===
			proxy.arr[2].getOriginalTarget()
	).toBe(true);
	...
	expect(
		proxy.newObj.subArr[1][0].getOriginalTarget() ===
			proxy.arr[3].getOriginalTarget()
	).toBe(true);
});
...
test("6. Observe on referenced changes and cloned changes", () => {
	...
	proxy.level1_1.on({
		...
		listener: function (change) {
			...
			if (changes.length === 3)
				expect(change.oldValue).toBe(changes[0].value); //same reference
		},
		deep: true,
	});
...
test("9. Comprehensive events of changes", () => {
	...
	proxy.on({
		...
		listener: function (change) {
			...
			expect(change.path).toBe(
				".level1_2.level2_1.level3_1.arr2[2][2][1].deep.new"
			);
	...
	proxy.level1_2.on({
		...
		listener: function (change) {
			...
			expect(change.value).toBe(17);
			expect(change.path).toBe(
				".level2_1.level3_1.arr2[2][2][1].deep.new"
			);
			expect(change.type).toBe("create");
		},
		deep: true,
	...
	proxy.level1_2.level2_1.level3_1.arr2[2][2][1].deep.on({
		...
		listener: function (change) {
			expect(this).toBe(
				proxy.level1_2.level2_1.level3_1.arr2[2][2][1].deep
			);
			expect(change.oldValue).toBe(undefined);
			expect(change.value).toBe(17);
	...
	//part 4
	let oldValue =
		proxy.level1_2.level2_1.level3_1.arr2[2][2].getOriginalTarget(); //[14, { deep: { deeper: 'abc' } }, 16];
		...
		listener: function (change) {
			...
			if (counter === 3)
				expect(change).toEqual({
					...
					value: { new: "new" },
					type: "create",
					path: ".level1_2.level2_1.level3_1.arr2[2][2]",
				});
			else if (counter === 6)
				expect(change).toEqual({
					oldValue: "new",
					value: [0, 1, 2, 3, 4, 5, 6],
					type: "update",
					path: ".level1_2.level2_1.level3_1.arr2[2][2].new",
				});
			else if (counter === 9)
				expect(change).toEqual({
					oldValue: 0,
					value: undefined,
					type: "delete",
					path: ".level1_2.level2_1.level3_1.arr2[2][2].new[0]",
				});
			else
				throw new Error(
					`shouldn't have gotten here on step #${counter}`
		...
		listener: function (change) {
			...
			if (counter === 2)
				expect(change).toEqual({
					...
					value: { new: "new" },
					type: "create",
					path: ".level2_1.level3_1.arr2[2][2]",
				});
			else if (counter === 5)
				expect(change).toEqual({
					oldValue: "new",
					value: [0, 1, 2, 3, 4, 5, 6],
					type: "update",
					path: ".level2_1.level3_1.arr2[2][2].new",
				});
			else if (counter === 8)
				expect(change).toEqual({
					oldValue: 0,
					value: undefined,
					type: "delete",
					path: ".level2_1.level3_1.arr2[2][2].new[0]",
				});
			else
				throw new Error(
					`shouldn't have gotten here on step #${counter}`
...
test("10. Events for future sub objects and primitives not yet created", () => {
	...
	proxy.on({
		...
		path: ".obj.1",
		listener: function (change) {
			//path-selector can be with either dots or squared parenthesis
			expect(change).toEqual({
				type: "update",
				path: "",
				oldValue: { 2: { 3: 654 } },
...
test("11. Assign unproxified sub-objects and listen to captureEmit events", () => {
	...
	proxy.level1_2.on({
		event: "change",
		path: ".level2_2.level3_1._$level4_1",
		listener: function (change) {
