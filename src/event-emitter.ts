...
import { EVENTS, ND, NID, NODE_STATUSES, PROXY_STATUSES } from "./globals";
...
function getProxyValue(dataNode: DataNode, property?: string): any {
	if (
		dataNode[ND].proxyNode &&
		dataNode[ND].proxyNode[NID].status === PROXY_STATUSES.alive
	) {
	...
	const aProperty = property
		? property
		: (splitPath(dataNode[ND].propertyPath)[0] as string); // my property on the parent

	const parentNode = dataNode[ND].parentNode;

	if (
		parentNode[ND].proxyNode &&
		parentNode[ND].proxyNode[NID].status === PROXY_STATUSES.alive
	) {
		return parentNode[ND].proxyNode[ND].proxy?.[aProperty]; // proxy or primitive via parent's proxy object
...
export function initEmitEvent(
	...
	if (
		oldValue === value || // no new change was made
		!dataNode[ND].proxyNode
	) {
	...
	const proxyNode = dataNode[ND].proxyNode;
	...
	let path = "";
	let childDataNode = dataNode;
	if (childDataNode[property]) {
		// changed a property which has its own data node on the tree
		childDataNode = childDataNode[property];
	} else {
	...
	if (!deferredEvents) {
		...
		stackTraceLog(childDataNode, change, trace);

		bubbleEmit(childDataNode, change, property);

		if (wasOldValueProxy || isValueProxy) {
			// old value or new value are proxy meaning they are objects with children
			captureEmit(childDataNode, change);
		}
	} else {
		deferredEvents.push({
...
function bubbleEmit(
	...
	const thisValue = getProxyValue(dataNode, property);
	...
	if (!dataNode[ND].parentNode[ND].isTreePrototype) {
...
function captureEmit(dataNode: DataNode, change: ChangeEvent): void {
	...
	for (const key of keys) {
		...
		if (subValue !== subOldValue) {
			...
			const childNode = dataNode[key];
			if (childNode[NID].status !== NODE_STATUSES.stopped) {
				const thisValue = getProxyValue(childNode, key);
				iterateAndEmit(
					childNode[ND].listeners.shallow,
					thisValue,
					subChange,
				);
			}
...
export function initFunctionEmitEvent(
	...
	if (dataNode[ND].deferredEvents) {
		...
		for (const event of dataNode[ND].deferredEvents!) {
			if (event.change.path === "") {
				...
				const thisValue = getProxyValue(event.dataNode);
				iterateAndEmit(
					event.dataNode[ND].listeners.shallow,
					thisValue,
					event.change,
				);
				iterateAndEmit(
					event.dataNode[ND].listeners.deep,
					thisValue,
					event.change,
				);
			}
